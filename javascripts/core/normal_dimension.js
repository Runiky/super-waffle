function getDimensionFinalMultiplier(tier) {
  ...
  if (tier == 8 && player.achievements.includes("r23")) multiplier = multiplier.times(5456465465465.1);
  else if (player.achievements.includes("r34")) multiplier = multiplier.times(67671.02);
...
function hasInfinityMult(tier) {
    ...
    function buyManyDimensionAutobuyer(tier, bulk) {
        ...
        var name = TIER_NAMES[tier];
        var cost = player[name + 'Cost'].times(1.1 - dimBought(tier))
        if (!player.break && player.money.gt(Number.MAX_VALUE)) return false;
